...
async def test_executes_sql_and_records_metrics() -> None:
    ...
    assert updates["subgraph_metrics"]["numerical.sql.executed"] == 1
    assert updates["subgraph_metrics"]["numerical.sql.rows"] == 1
    assert updates["numerical_result_metrics"]["table_aliases"] == ["gdp"]
    assert updates["numerical_result_metrics"]["sql_query"].startswith("SELECT country")
    trace = updates["numerical_trace"]
    assert trace["executor"]["row_count"] == 1
    assert trace["executor"]["sql_query"].startswith("SELECT country")
