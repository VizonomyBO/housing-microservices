...
def mock_runner():
    runner = MagicMock()
    runner.run_chat = AsyncMock(return_value=ChatRunResult(
        done_payload={"answer": "## Mocked content for section"},
        messages=[],
        tool_calls=[]
    ))
    return runner
...
async def test_generate_housing_report_logic(
    ...
    with patch("agent_api.services.reports.DocumentRepository") as MockDocRepo, \
