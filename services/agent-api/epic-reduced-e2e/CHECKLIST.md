...
11. - [ ] [Task 11 — LangGraph Runner & Full Ingestion Integration](tasks/task-11-langgraph-runner-integration.md) *(2025-12-03 10:40 PT: Still blocked — repo continues to lack LangGraph graph definitions, ingestion/embedding helpers, or OpenAI/Voyage clients. `pyproject.toml` has no `langgraph`, `openai`, or `voyageai` dependency; there are no shared chunkers/`embedding_writer` modules in `packages/`; `DocumentUploadService` only writes stub text chunks; and composer/validator protocols in the nodes folder have no implementations to call. Need upstream guidance or shared modules before wiring `/v1/chat` to “real tools.”)*
