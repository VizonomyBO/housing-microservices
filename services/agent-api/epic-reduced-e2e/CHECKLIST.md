...
11. - [ ] [Task 11 — LangGraph Runner & Full Ingestion Integration](tasks/task-11-langgraph-runner-integration.md) *(2025-12-03: Blocked — repo still lacks a LangGraph runner, ingestion/embedding helpers, and any concrete OpenAI/Voyage client implementations. `pyproject.toml` has no `langgraph`/`openai`/`voyageai` deps, `src/agent_api/http/streaming.py` only exposes `UnconfiguredChatRunner`, `DocumentUploadService` writes stub chunks with no embeddings, and nodes like `AnswerSynthesizerNode`/`CitationVerifierNode` reference protocols with no concrete composer/validator implementations, so there is nothing to wire into `/v1/chat` yet.)*
