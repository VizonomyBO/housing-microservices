...
class TestUserService:
    ...
    def test_update_user_first_name(self, db_session, sample_user):
        ...
        user, error = UserService.update_user(sample_user.user_id, {"first_name": "Jane"})
        assert error is None
        assert user.first_name == "Jane"

    def test_update_user_invalid_email(self, db_session, sample_user):
        """Test updating user with invalid email"""
        user, error = UserService.update_user(sample_user.user_id, {"email": "invalid-email"})
        assert user is None
    ...
    def test_update_user_duplicate_email(self, db_session, sample_user):
        ...
        user, error = UserService.update_user(sample_user.user_id, {"email": "other@example.com"})
        assert user is None
    ...
    def test_update_user_invalid_role(self, db_session, sample_user):
        ...
        user, error = UserService.update_user(sample_user.user_id, {"role": "invalid_role"})
        assert user is None
    ...
    def test_get_all_users_filter_by_role(self, db_session, sample_user, admin_user):
        ...
        result = UserService.get_all_users(role="admin")
