...
async def download_document_by_name(
    ...
    Requires authentication. Searches S3 across all folders to find the first
    document with a matching canonical name and downloads it.
    """
    # Authenticate user
    user = await _require_user(authorization, settings)
    ...
    try:
        # Search S3 directly across all folders for matching canonical name
        file_bytes = find_and_download_pdf_by_name(
        ...
        if "not found" in error_msg.lower():
            raise HTTPException(
                status_code=404,
                detail=f"PDF file not found in S3 with canonical name '{canonical_name}'",
            ) from exc
        raise HTTPException(
            status_code=500, detail=f"Failed to download PDF: {error_msg}"
        ) from exc
    except Exception as exc:
        logger.exception("Unexpected error during PDF download by name")
        raise HTTPException(
            status_code=500, detail="Unexpected error during download"
        ) from exc
