...
- AWS smoke (AWS mode only): `ENV_FILE=.env.prod bash scripts/prod_smoke_check.sh | tee /tmp/prod_smoke_task13.log`
...
- Post-deploy smoke: `ENV_FILE=.env.prod bash scripts/prod_smoke_check.sh | tee /tmp/prod_smoke_task13_postdeploy.log` (docs `b8f622e3-abcb-4d7c-a601-868ca4bf50af`, `7f775c98-faff-4490-8b9f-9bc2bcd2596a`, `8b1047c7-fa1e-4bef-8d2d-b47d7bade427`; `requires_sql:true` on ledger/KPI prompts, partial on mixed prompt).
