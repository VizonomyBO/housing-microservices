...
def upgrade() -> None:
    ...
    op.create_table('pillar_answers',
        ...
        sa.ForeignKeyConstraint(['document_id'], ['documents.id'], name=op.f('fk_pillar_answers_document_id_documents')),
        sa.ForeignKeyConstraint(['owner_user_id'], ['users.id'], name=op.f('fk_pillar_answers_owner_user_id_users')),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_pillar_answers'))
    )

    # 2. Create pillar_answer_sources table
    op.create_table('pillar_answer_sources',
        ...
        sa.Column('id', sa.UUID(), nullable=False),
        sa.ForeignKeyConstraint(['chunk_id'], ['chunks.id'], name=op.f('fk_pillar_answer_sources_chunk_id_chunks')),
        sa.ForeignKeyConstraint(['pillar_answer_id'], ['pillar_answers.id'], name=op.f('fk_pillar_answer_sources_pillar_answer_id_pillar_answers')),
        sa.PrimaryKeyConstraint('id', name=op.f('pk_pillar_answer_sources'))
    )

    # 3. Add embedding to graph_entities
    op.add_column('graph_entities', sa.Column('embedding', pgvector.sqlalchemy.vector.VECTOR(dim=512), nullable=True))
    ...
    op.add_column('workflow_edges', sa.Column('transition_type', sa.String(), nullable=False, server_default='success'))
    ...
    op.alter_column('chunks', 'embedding', type_=pgvector.sqlalchemy.vector.VECTOR(dim=1024), existing_type=pgvector.sqlalchemy.vector.VECTOR(dim=1536), postgresql_using='embedding::vector(1024)')


def downgrade() -> None:
    # 6. Revert chunks.embedding dimension
    op.alter_column('chunks', 'embedding', type_=pgvector.sqlalchemy.vector.VECTOR(dim=1536), existing_type=pgvector.sqlalchemy.vector.VECTOR(dim=1024), postgresql_using='embedding::vector(1536)')
