...
from shared_data_layer.db.models.retrieval import PillarAnswer, PillarAnswerSource, Chunk
from shared_data_layer.db.models.knowledge_graph import GraphEntity, GraphEdge
from shared_data_layer.db.models.workflow import WorkflowEdge, WorkflowVersion, WorkflowNode, WorkflowGraph
...
async def test_graph_entity_embedding(db_session: AsyncSession):
    ...
    # pgvector returns numpy array or list? usually list or object that behaves like list
    # But we just want to check it's not None and has correct dim
    assert fetched_entity.embedding is not None
    # assert len(fetched_entity.embedding) == 512 # Might need to cast to list if it's a Vector object
