...
from shared_data_layer.testing.factories.documents import DocumentFactory, ChunkFactory
from shared_data_layer.testing.factories.knowledge_graph import GraphEntityFactory, GraphEdgeFactory
from shared_data_layer.testing.factories.workflow import WorkflowGraphFactory, WorkflowVersionFactory, WorkflowNodeFactory, WorkflowEdgeFactory
...
class TestRepositories:
    ...
    async def test_document_repository(self, db_session: AsyncSession):
        ...
        doc = await DocumentFactory.create_async(session=db_session, country_code="US", chunks=[])
        ...
        doc_uk = await DocumentFactory.create_async(session=db_session, country_code="UK", chunks=[])
        doc_uk_id = doc_uk.id
    ...
    async def test_knowledge_graph_repository(self, db_session: AsyncSession):
        ...
        # Entity A -> Entity B
        entity_a = await GraphEntityFactory.create_async(session=db_session, name="Entity A", country_code="US")
        entity_b = await GraphEntityFactory.create_async(session=db_session, name="Entity B", country_code="US")
        ...
        # We need to refresh/load relationships if factory didn't do it fully for the repo query to work? 
        ...
        entity_c = await GraphEntityFactory.create_async(session=db_session, country_code="UK")
        entity_d = await GraphEntityFactory.create_async(session=db_session, country_code="UK")
        edge_uk = await GraphEdgeFactory.create_async(session=db_session, source=entity_c, target=entity_d, relation="related_to")
    ...
    async def test_workflow_repository(self, db_session: AsyncSession):
        ...
        # Unpublished version
        v1 = await WorkflowVersionFactory.create_async(
            session=db_session, 
            graph=graph, 
            version_number=1, 
            is_published=False,
