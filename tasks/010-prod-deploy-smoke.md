# Task: Production Deploy + Smoke Script
...
## Objective
- Create a deployment script that provisions/updates the ingestion-first stack on prod (agent-api, ingestion-service, auth-service, user-service, Postgres) with correct ports/envs, then run the full smoke (ingestion upload → poll → attach → chat) end-to-end. Iterate until every component runs correctly; execute the smoke flow when ready.
